...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateDataTable () {
    dataTable.innerHTML = '';
    const timeSet = new Set();
    datasets.forEach(ds => ds.rawData.filter(p => p.x != null).forEach(p => { let _a; return timeSet.add((_a = p.x) !== null && _a !== void 0 ? _a : -1); }));
    const times = Array.from(timeSet).sort((a, b) => a - b);
    ...
    const colNames = datasets.map(ds => { let _a, _b; return `<th title="${(_a = ds.fullLabel) !== null && _a !== void 0 ? _a : '?'}">${truncateLabel((_b = ds.label) !== null && _b !== void 0 ? _b : '?', 15)}</th>`; });
